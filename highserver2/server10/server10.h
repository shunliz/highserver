...
#define MAX_BUF_SIZE 1024 /* ulimit -n opened FDs per process.记得修改哦，否则还是select效果,就不是epoll效果了哦，呵呵*/
#define BLOCKING_SEND_TIMEOUT 20

typedef void (*CSocketLifecycleCallback)(int cs,int lifecycle);//lifecycle:0:OnAccepted,-1:OnClose typedef int (*BlockingSender_t)(void * senderBase,int cs, void * buf, size_t nbs);
...
typedef struct {
	...
	int workerWaitTimeout;//wait timeout volatile
	int workerCounter;
